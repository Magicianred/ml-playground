...
const prepareTrainingData = () => {
  ...
  */
  ...
  // What if AccuracyCheckExamples were all the unique combos of test testValues?
  const set = new Set(trainingExamples.map(JSON.stringify));
  const uniqueTrainingCombos = Array.from(set).map(JSON.parse);
  console.log("uniqueTrainingCombos", uniqueTrainingCombos);
  store.dispatch(setAccuracyCheckExamples(uniqueTrainingCombos));
